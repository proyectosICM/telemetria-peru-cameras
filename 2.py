        ...
        while not reader.at_eof():
            ...
            while True:
                ...
                try:
                    ...
                    # 🔎 DEBUG: log de frame recibido
                    logger.info(
                        f"[FRAME] msgId=0x{msg_id.hex()} "
                        f"phone={hdr['phone_str']} "
                        f"body_len={hdr['body_len']} "
                    ...
                    if handler:
                        ...
                        if msg_id == b"\x01\x02":
                            if ASK_LOCATION_AFTER_AUTH:
                                ...
                                logger.info(
                                    f"→ CMD 0x8201 Location Query to "
                                    f"{hdr['phone_str']}"
                                )
                                writer.write(cmd)
                                await writer.drain()
                            if START_TEMP_TRACKING_AFTER_AUTH:
                                ...
                                logger.info(
                                    f"→ CMD 0x8202 Temp Tracking to "
                                    f"{hdr['phone_str']} "
                                    f"({TEMP_TRACK_INTERVAL_S}s x "
                                    f"{TEMP_TRACK_DURATION_MIN}min)"
                                )
                                writer.write(cmd2)
                                await writer.drain()
                            ...
                            logger.info(
                                f"→ CMD 0x9102 Control START ch={VIDEO_CHANNEL}"
                            )
